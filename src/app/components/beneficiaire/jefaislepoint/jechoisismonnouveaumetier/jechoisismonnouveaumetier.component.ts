...
@Component({
  selector: 'app-jechoisismonnouveaumetier',
  templateUrl: './jechoisismonnouveaumetier.component.html',
  styleUrls: ['./jechoisismonnouveaumetier.component.css']
})
export class JechoisismonnouveaumetierComponent implements OnInit {

    
  public currentstepvalid:boolean=false;
  public ouinon : SelectItem[];
  public jechoisismonnouveaumetierjeveuxensavoirplus: string[] = [];
  ...
  ngOnInit() {
      ...
      if (this.activatedroute.snapshot.params['step']!==undefined) {
          if (this.activatedroute.snapshot.params['step'].toString()!==""){
              this.navigationservice.jechoisismonnouveaumetiercurrentstep=Number(this.activatedroute.snapshot.params['step']);
          }
      }
  }
  ...
  public checkifstepvalid():void{
      ...
      switch (this.navigationservice.jechoisismonnouveaumetiercurrentstep) {
          default:
  ...
  public goonestepfurther():void {
      this.navigationservice.jechoisismonnouveaumetiercurrentstep = this.navigationservice.jechoisismonnouveaumetiercurrentstep + 1;
      this.checkifstepvalid();
      console.log("currentstep: "+this.navigationservice.jechoisismonnouveaumetiercurrentstep);
      if (this.navigationservice.jechoisismonnouveaumetiercurrentstep === this.navigationservice.jechoisismonnouveaumetiernumberofsteps) {
          this.conclude();
      }
  } 
  ...
  public conclude():void{
      this.readwritebufferservice.updatebuffer("etapejechoisismonnouveaumetier","done","update");
  ...
  public setstepatbeginning():void {
      let step : number = 0;
      step = Math.max(this.dbuserinfoservice.userinfo.experience.jechoisismonnouveaumetierstepvalidation);
      this.navigationservice.jechoisismonnouveaumetiercurrentstep = step + 1;//if step == null, go to first step, if step == 2, go to the one after, and so on
  }
