...
@Component({
    selector: 'app-jemerenseignesurmonmetier',
    templateUrl: './jemerenseignesurmonmetier.component.html',
    styleUrls: ['./jemerenseignesurmonmetier.component.css']
})
export class JemerenseignesurmonmetierComponent implements OnInit {
    
    
    public currentstepvalid:boolean=false;
    public ouinon : SelectItem[];
    public jemerenseignesurmonmetierjeveuxensavoirplus: string[] = [];
    ...
    ngOnInit() {
        ...
        if (this.activatedroute.snapshot.params['step']!==undefined) {
            if (this.activatedroute.snapshot.params['step'].toString()!==""){
                this.navigationservice.jemerenseignesurmonmetiercurrentstep=Number(this.activatedroute.snapshot.params['step']);
            }
        }
    }
    ...
    public checkifstepvalid():void{
        ...
        switch (this.navigationservice.jemerenseignesurmonmetiercurrentstep) {
            default:
    ...
    public goonestepfurther():void {
        this.navigationservice.jemerenseignesurmonmetiercurrentstep = this.navigationservice.jemerenseignesurmonmetiercurrentstep + 1;
        this.checkifstepvalid();
        console.log("currentstep: "+this.navigationservice.jemerenseignesurmonmetiercurrentstep);
        if (this.navigationservice.jemerenseignesurmonmetiercurrentstep === this.navigationservice.jemerenseignesurmonmetiernumberofsteps) {
            this.conclude();
        }
    } 
    ...
    public conclude():void{
        this.readwritebufferservice.updatebuffer("etapejemerenseignesurmonmetier","done","update");
        this.readwritebufferservice.transmitbuffer();
    }
    ...
    public setstepatbeginning():void {
        let step : number = 0;
        step = Math.max(this.dbuserinfoservice.userinfo.experience.jemerenseignesurmonmetierstepvalidation);
        this.navigationservice.jemerenseignesurmonmetiercurrentstep = step + 1;//if step == null, go to first step, if step == 2, go to the one after, and so on
    }
