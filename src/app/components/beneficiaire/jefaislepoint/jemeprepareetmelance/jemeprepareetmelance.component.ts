...
@Component({
    selector: 'app-jemeprepareetmelance',
    templateUrl: './jemeprepareetmelance.component.html',
    styleUrls: ['./jemeprepareetmelance.component.css']
})
export class JemeprepareetmelanceComponent implements OnInit {
    
    
    public currentstepvalid:boolean=false;
    public ouinon : SelectItem[];
    public jemeprepareetmelancejeveuxensavoirplus: string[] = [];
    ...
    ngOnInit() {
        ...
        if (this.activatedroute.snapshot.params['step']!==undefined) {
            if (this.activatedroute.snapshot.params['step'].toString()!==""){
                this.navigationservice.jemeprepareetmelancecurrentstep=Number(this.activatedroute.snapshot.params['step']);
            }
        }
    }
    ...
    public checkifstepvalid():void{
        ...
        switch (this.navigationservice.jemeprepareetmelancecurrentstep) {
            default:
    ...
    public goonestepfurther():void {
        this.navigationservice.jemeprepareetmelancecurrentstep = this.navigationservice.jemeprepareetmelancecurrentstep + 1;
        this.checkifstepvalid();
        console.log("currentstep: "+this.navigationservice.jemeprepareetmelancecurrentstep);
        if (this.navigationservice.jemeprepareetmelancecurrentstep === this.navigationservice.jemeprepareetmelancenumberofsteps) {
            this.conclude();
        }
    } 
    ...
    public conclude():void{
        this.readwritebufferservice.updatebuffer("etapejemeprepareetmelance","done","update");
        this.readwritebufferservice.transmitbuffer();
    }
    ...
    public setstepatbeginning():void {
        let step : number = 0;
        step = Math.max(this.dbuserinfoservice.userinfo.experience.jemeprepareetmelancestepvalidation);
        this.navigationservice.jemeprepareetmelancecurrentstep = step + 1;//if step == null, go to first step, if step == 2, go to the one after, and so on
    }
 }
