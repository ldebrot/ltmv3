...
@Component({
    selector: 'app-jedeveloppeetactivemonreseau',
    templateUrl: './jedeveloppeetactivemonreseau.component.html',
    styleUrls: ['./jedeveloppeetactivemonreseau.component.css']
})
export class JedeveloppeetactivemonreseauComponent implements OnInit {
    
   
    public currentstepvalid:boolean=false;
    public ouinon : SelectItem[];
    public jedeveloppeetactivemonreseaujeveuxensavoirplus: string[] = [];
    ...
    ngOnInit() {
        ...
        if (this.activatedroute.snapshot.params['step']!==undefined) {
            if (this.activatedroute.snapshot.params['step'].toString()!==""){
                this.navigationservice.jedeveloppeetactivemonreseaucurrentstep=Number(this.activatedroute.snapshot.params['step']);
            }
        }
    }
    ...
    public checkifstepvalid():void{
        ...
        switch (this.navigationservice.jedeveloppeetactivemonreseaucurrentstep) {
            default:
    ...
    public goonestepfurther():void {
        this.navigationservice.jedeveloppeetactivemonreseaucurrentstep = this.navigationservice.jedeveloppeetactivemonreseaucurrentstep + 1;
        this.checkifstepvalid();
        console.log("currentstep: "+this.navigationservice.jedeveloppeetactivemonreseaucurrentstep);
        if (this.navigationservice.jedeveloppeetactivemonreseaucurrentstep === this.navigationservice.jedeveloppeetactivemonreseaunumberofsteps) {
            this.conclude();
        }
    } 
    ...
    public conclude():void{
        this.readwritebufferservice.updatebuffer("etapejedeveloppeetactivemonreseau","done","update");
        this.readwritebufferservice.transmitbuffer();
    }
    ...
    public setstepatbeginning():void {
        let step : number = 0;
        step = Math.max(this.dbuserinfoservice.userinfo.experience.jedeveloppeetactivemonreseaustepvalidation);
        this.navigationservice.jedeveloppeetactivemonreseaucurrentstep = step + 1;//if step == null, go to first step, if step == 2, go to the one after, and so on
    }
